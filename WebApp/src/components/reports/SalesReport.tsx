...
import { Download, TrendingUp, Undo2, Receipt, Percent } from "lucide-react";
...
export default function SalesReport({ fromDate, toDate }: { fromDate: string; toDate: string }) {
  ...
  if (loadingSummary) {
    return <div className="glass rounded-2xl p-10 text-center text-gray-400">กำลังโหลด...</div>;
  }
      ...
      <ChartCard title="ยอดขายรายชั่วโมง" subtitle={`วันที่ ${toDate}`}>
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={hourlyChart}>
            ...
            <YAxis tick={{ fontSize: 11 }} tickLine={false} axisLine={false} width={70} />
            <Tooltip contentStyle={TOOLTIP_STYLE} formatter={(value) => [money(value), "ยอดขาย"]} />
