...
Modules: `auth`, `users`, `categories`, `products`, `stock`, `orders`, `reports`, `settings`. All routes are mounted under `/api/<module>` in `WebAPI/src/app.ts`. Auth is JWT bearer token (`src/lib/jwt.ts`, `src/middleware/auth.ts` sets `req.user = { id, role }`); role gate is `src/middleware/requireRole.ts` with two roles: `ADMIN`, `CASHIER`.
...
Route groups: `(auth)/login` (public) and `(main)/*` (dashboard, pos, products, orders, settings) wrapped by `WebApp/src/app/(main)/layout.tsx`, which redirects to `/login` if unauthenticated once the auth store is `initialized`.
